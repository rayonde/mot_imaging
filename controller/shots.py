...
class ShotPresenter:
    ...
    def process_shot(self, name, paths):
        ...
        self.queue.put((self.display_shot, (shot,)))  # Display raw absorption image

        if config.fit:
            shot.run_fit(config)
            self.queue.put((self.display_shot, (shot,)))  # Display fit overlay

        # Save to png output
        output_file_path = self._output_path(name)
        self._save_output_image(shot, output_file_path)

        # Saves fit params to log file
        cmnts = self.settings_view.get_comment()
        logging.info("Updating logging.csv for shot %s with comment %s " % (name, cmnts))
        self._write_to_log_file(name, shot, cmnts)
    ...
    def _update_recent_shots(self, shot):
        self.recent_shots.appendleft(shot)

    def _save_output_image(self, shot, output_file_path):
        figure = Figure(figsize=(8, 5))
        shot.plot(figure)
        figure.savefig(output_file_path, dpi=150)

    def _write_to_log_file(self, name, shot, cmnts):
        # Sets the path directory for generating a log file in hdf5 format in the raw data folder
        output_file_path = self._output_log_path(name)

        with h5py.File(output_file_path, "a") as logfile:
            lf = logfile.create_group(str(time.strftime('%H:%M:%S')))  # creates group based on 24hr timestamp
            lf.create_dataset("atom", data=shot.data)
            lf.create_dataset("beam", data=shot
